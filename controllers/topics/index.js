...
exports.updateCardBucket = async (req, res) => {
  try {
    const {
      body: { cardId, increment },
      params: { subjectId, topicId },
    } = req;
    ...
    const updatedSubject = await Subject.findOneAndUpdate(
      ...
      {
        $inc: {
          "topics.$[topic].cards.$[card].bucket": increment,
        },
        $set: {
          "topics.$[topic]._id": topicId,
        },
    ...
    return res.json("Update card bucket working");
  } catch (error) {
    console.log("EDIT CARD BUCKET FAILED WITH ERROR: ", error);
    return res.status(422).send(error.message);
  }
};
